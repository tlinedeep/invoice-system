...
with app.app_context():
    try:
        ...
        cols = [c["name"] for c in insp.get_columns("receiving_notes")]
        if "recipient" not in cols:
            db.session.execute(sa_text("ALTER TABLE receiving_notes ADD COLUMN recipient VARCHAR(50)"))
            db.session.commit()
            print("[迁移] receiving_notes 添加 recipient 列")
    except Exception as e:
        print(f"[迁移跳过] {e}")
